...
#######################################################
def fitPredictAndTest(orgUsers,yRes,featureList,category,predictMethod,usePrints=False,random_state=1,normalize=True,crossPredict=False):
    users = orgUsers.copy(deep=True)
    initiateUsers(users)
    X = sortFeatures(users,featureList)
    X_byteDF = category2binaryFeatures(X, usePrints=usePrints)
    y = sortResults(yRes,category)
...
def getXbyte(orgUsers,yRes,featureList,category,normalize=True, usePrints=False):
    users = orgUsers.copy(deep=True)
    initiateUsers(users)
    X = sortFeatures(users,featureList)
    X_byteDF = category2binaryFeatures(X, usePrints=usePrints)
...
class featureList():
    def __init__(self, myList = ('affiliate_channel','affiliate_provider','first_affiliate_tracked',
                                 ...
                                 'signup_method','signup_flow', 'date_account_created_dayofweek',
