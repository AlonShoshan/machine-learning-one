...
###########################################################
def loadAndUpdateFeatures(path, featureStruct=featUp.generateFeatureStruct()):
...
#######################################################
def fitPredictAndTest(users,featureList,category,predictMethod,usePrints=False,random_state=1,normalize=True,crossPredict=False):
    initiateUsers(users)
...
def normDf(myDF, minNum = -1, maxNum = 1):
    ...
    for column in myDF.columns:
        myDF[column] = myRange * myDF[column]/myDF[column].max() + minNum    
...
def category2binaryFeatures(users, usePrints=False):
    ...
    for feature, dtype in Xlabel.dtypes.iteritems():
        if dtype.name != 'category':
            nonCategoryArr.append(Xlabel[[feature]])
            Xlabel = Xlabel.drop(feature, 1)
    ...
    for i in nonCategoryArr:
        feature = i.keys()[0]
        sCoulumn = getattr(i,feature)
        X_byte = concatenate((X_byte,sCoulumn.values.reshape(1,len(i[[feature]])).transpose()),axis=1)
    ...
    X_byteDF = makeDataFrame(X_byte,labels,n_values,nonCategoryArr)
    return X_byteDF

# Combine the category features with the non-category into a dataframe
def makeDataFrame(X_byte,labels,n_values,nonCategoryArr):
...
class featureList():
    ...
    def add(self, myList):
        for i in myList:
            self.list.append(i)
    def addByRegex(self, regexList, users):
        for regex in regexList:
            for i in users.keys():
                if re.match(r'(.*)' + regex + r'(.*)',i):
                    self.list.append(i)
