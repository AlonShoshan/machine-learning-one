...
class ageGenderModel:
    ...
    def sortAge(self, users):
        if ((users.columns == 'validAge').any()): # if users have validAge bit
            X_nan_age = users[users.validAge == -1].drop(['age', 'validAge'], 1)
    ...
    def sortGender(self,users):
        X_nan_gender = users[users.gender_0 == 1].drop('gender_0', 1)
        X_gender = users[users.gender_0 == -1].drop('gender_0', 1)
        for i in range(5):
            if ((X_nan_gender.columns == ('gender_' + str(i))).any()) :
                X_nan_gender = X_nan_gender.drop([('gender_'+str(i))], 1)
